...
public class LayoutDeserializationTests {
    ...
    @Before
    public void setUp() {
        objectMapper.getSubtypeResolver().registerSubtypes(AccessJsonLayoutBaseFactory.class, JsonLayoutBaseFactory.class);
    }
    ...
    public void testDeserializeJson() throws Exception {
        ...
        assertThat(layout).isInstanceOf(JsonLayoutBaseFactory.class);
        JsonLayoutBaseFactory jsonLayoutBaseFactory = (JsonLayoutBaseFactory) layout;
        assertThat(jsonLayoutBaseFactory).isNotNull();
        assertThat(jsonLayoutBaseFactory.isPrettyPrint()).isTrue();
        assertThat(jsonLayoutBaseFactory.isIncludeStackTrace()).isTrue();
        assertThat(jsonLayoutBaseFactory.getTimestampFormat()).isEqualTo("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'");
    }
    ...
    public void testDeserializeAccessJson() throws Exception {
        ...
        AccessJsonLayoutBaseFactory accessJsonLayoutBaseFactory = (AccessJsonLayoutBaseFactory) layout;
        assertThat(accessJsonLayoutBaseFactory.getTimestampFormat()).isEqualTo("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'");
    }
    ...
    public void testLogAccessJsonToConsole() throws Exception {
        ConsoleAppenderFactory<IAccessEvent> consoleAppenderFactory = factory.build(new File(Resources.getResource("yaml/json-access-log.yml").toURI()));
        ...
        try {
            ...
            when(request.getRemoteHost()).thenReturn("crawler.example.com");
        ...
        assertThat(jsonNode.get("remoteHost").asText()).isEqualTo("crawler.example.com");
        assertThat(jsonNode.get("headers").get("Connection").asText()).isEqualTo("keep-alive");
        assertThat(jsonNode.get("headers").get("User-Agent").asText()).isEqualTo("Mozilla/5.0");
